    ...
    async function fetchDashboardData() {
        ...
        try {
            ...
            const { data: legacyMatch } = await supabase
                .from('patients')
                .select(`
                    id, status, weight, height, birth_date, gender, activity_level, patient_goals,
                    program_templates (
    ...
    return (
        <div className="space-y-6">
            ...
            <Tabs defaultValue="overview" className="space-y-6">
                <TabsList className="grid w-full grid-cols-4 lg:w-[600px] mb-4 bg-gray-100/50 p-1 rounded-xl">
                    <TabsTrigger value="overview" className="text-xs sm:text-sm rounded-lg data-[state=active]:bg-emerald-500 data-[state=active]:text-white transition-colors"><LayoutDashboard className="h-4 w-4 mr-1 sm:mr-2" />Özet</TabsTrigger>
                    <TabsTrigger value="labs" className="text-xs sm:text-sm rounded-lg data-[state=active]:bg-blue-500 data-[state=active]:text-white transition-colors"><Beaker className="h-4 w-4 mr-1 sm:mr-2" />Tahliller</TabsTrigger>
                    <TabsTrigger value="imaging" className="text-xs sm:text-sm rounded-lg data-[state=active]:bg-purple-500 data-[state=active]:text-white transition-colors"><Camera className="h-4 w-4 mr-1 sm:mr-2" />Görüntüleme</TabsTrigger>
                    <TabsTrigger value="measurements" className="text-xs sm:text-sm rounded-lg data-[state=active]:bg-orange-500 data-[state=active]:text-white transition-colors"><Ruler className="h-4 w-4 mr-1 sm:mr-2" />Ölçümler</TabsTrigger>
                </TabsList>
