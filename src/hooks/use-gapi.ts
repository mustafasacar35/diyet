...
export function useGapi(enabled: boolean = false) {
    ...
    const initClient = useCallback(async (apiKey: string, clientId: string) => {
        ...
        try {
            ...
            try {
                ...
                appendLog(`GAPI Client Init Failed (Non-critical for Login): ${errMsg}`);
                setError('API Başlatma Hatası: ' + errMsg);
                // Create token client anyway, so we can try to Auth (maybe API key issue is separate)
            }
            ...
            try {
                ...
                if (client) {
                    ...
                    tokenClientRef.current = client; // Set ref immediately
                ...
                setError("Token Client Hatası: " + tokenErr.message);
            }
    ...
    const login = useCallback(() => {
        ...
        if (client) {
            ...
            appendLog('TokenClient not ready - check if initClient was called first.');
            alert("Google servisi henüz hazır değil. Lütfen 'Ayarlar' panelindeki API Loglarına bakın.");
        }
