...
export class Planner {
    ...
    async generateWeeklyPlan(
                    ...
                    for (const slotName of topUpSlots) {
                        ...
                        while (currentItems.length < config.maxItems && topUpGuard < 5) {
                            ...
                            if (extraFood) {
                                ...
                                dailyContext.selectedFoods.push(extraFood)
                    ...
                    for (const rule of nutritionalRules) {
                        ...
                        if (conditionMet && def.action.type === 'add') {
                            ...
                            for (let attempt = 0; attempt < candidateFoodIds.length; attempt++) {
                                ...
                                if (!hasCollision) { const mockDayMeals = Object.keys(slots).flatMap(s => slots[s]); if (this.hasAffinityConflict(foodToAdd, mockDayMeals, targetSlot)) { hasCollision = true; this.log(i + 1, targetSlot, 'info', `Nutritional Rule '${rule.name}': Skipped ${foodToAdd.name} — affinity conflict`) } }
        ...
        // ========= AFFINITY POST-VALIDATION SWEEP =========
        // Final safety net: remove any foods that violate forbidden affinity rules
        {
            const slotsPerDay = new Map();
            for (const meal of plan.meals) {
                const key = meal.day + '|' + meal.slot;
                if (!slotsPerDay.has(key)) slotsPerDay.set(key, []);
                slotsPerDay.get(key).push(meal);
            }
            
            const mealsToRemove = new Set();
            let removedCount = 0;
            
            for (const [key, slotMeals] of slotsPerDay) {
                for (let i = slotMeals.length - 1; i >= 0; i--) {
                    const meal = slotMeals[i];
                    if (!meal.food || mealsToRemove.has(meal)) continue;
                    
                    const dayNum = meal.day;
                    const dayMeals = plan.meals
                        .filter((m: any) => m.day === dayNum && m !== meal && !mealsToRemove.has(m))
                        .map((m: any) => ({ slot: m.slot, food: m.food }));
                    
                    if (this.hasAffinityConflict(meal.food, dayMeals, meal.slot)) {
                        mealsToRemove.add(meal);
                        removedCount++;
                        this.log(dayNum, meal.slot, 'info', '[POST-VALIDATION] Removed affinity-violating food: ' + meal.food.name);
                    }
                }
            }
            
            if (mealsToRemove.size > 0) {
                plan.meals = plan.meals.filter((m: any) => !mealsToRemove.has(m));
                this.log(0, 'VALIDATION', 'info', 'Removed ' + removedCount + ' affinity-violating foods from the final plan.');
            }
        }
                ...
                while (currentCount < maxCount && flexCount < MAX_FLEX_ITERATIONS) {
                    ...
                    for (const slotName of scopeMeals) {
                        ...
                        const candidates = this.eligibleFoods.filter((f: any) => {
                            ...
                            if (this.hasAffinityConflict(f, plan.meals.filter((m: any) => m.day === bestDay.day), slotName)) return false
                ...
                for (let day = 1; day <= 7; day++) {
                    ...
                    for (const slotName of mealTypes) {
                        ...
                        if (slotMeals.length < (config.maxItems || 4)) {
                            ...
                            if (!this.hasTagConflict(food, slotTags) && !this.hasAffinityConflict(food, dayMeals, slotName)) {
                        ...
                        if (slotMeals.length >= (config.maxItems || 4)) {
                            ...
                            for (const meal of slotMeals) {
                                ...
                                if (this.hasAffinityConflict(food, dayMeals.filter((m: any) => m !== meal), slotName)) continue
    ...
    private async selectFoodsForSlot(
        ...
        for (const foodName of fixedFoods) {
            ...
            if (food) {
                ...
                // === TEMP TRACKING ===
                const _ft = Array.isArray(food.tags) ? food.tags.map((t: any) => String(t).toLowerCase()) : [];
                if (_ft.includes('tost') || _ft.includes('ekmek') || (food.name || '').toLowerCase().includes('ekmek') || (food.name || '').toLowerCase().includes('pita')) {
                    console.log('[SEL-TRACK] FOOD ADDED TO SLOT: "' + food.name + '" tags=[' + _ft.join(',') + '] | slot="' + slotName + '" | selectedFoods count=' + selectedFoods.length + ' | slotSelectedFoods===selectedFoods? ' + (context.slotSelectedFoods === selectedFoods));
                }
        ...
        if ((proteinRatio < 0.7 || fatRatio < 0.7) && caloriesRemaining > 50 && selectedFoods.length < config.maxItems) {
            ...
            const fillerDayMeals = selectedFoods.map(f => ({ slot: slotName, food: f }))
            if (fillerFood && fillerFood.calories <= caloriesRemaining * 1.3 && !this.hasAffinityConflict(fillerFood, fillerDayMeals, slotName)) {
        ...
        if (selectedFoods.length < config.minItems) {
            ...
            while (selectedFoods.length < config.minItems && guard < 16) {
                ...
                if (!added && guard > emergencyRoles.length) {
                    const emergencyDayMeals = selectedFoods.map(f => ({ slot: slotName, food: f }));
                    const fallback = this.allFoods.find((f: any) => !selectedIds.has(f.id) && f.role !== 'mainDish' && f.role !== 'breakfast_main' && !this.hasAffinityConflict(f, emergencyDayMeals, slotName));
                    if (fallback) {
    ...
    private async selectBestFoodByRole(
        ...
        let candidates = this.eligibleFoods.filter(f => {
            ...
            if (this.hasTagConflict(f, slotTags)) return false
                // HARD AFFINITY FILTER: Block foods that would violate forbidden affinity rules
                {
                    const _existingSlotFoods = (context.slotSelectedFoods || []).map((sf: any) => ({ slot: context.slotName || '', food: sf }));
                    const _existingDayFoods = (context.selectedFoods || []).map((sf: any) => ({ slot: '', food: sf }));
                    const _allMeals = [..._existingDayFoods, ..._existingSlotFoods];
                    if (this.hasAffinityConflict(f, _allMeals, context.slotName || '')) return false
                }
        ...
        if (candidates.length === 0) {
            ...
            candidates = this.eligibleFoods.filter(f => {
                ...
                if (excludeIds.has(f.id)) return false
                if (this.hasTagConflict(f, slotTags)) return false
                // HARD AFFINITY FILTER (fallback path)
                {
                    const _existingSlotFoods = (context.slotSelectedFoods || []).map((sf: any) => ({ slot: context.slotName || '', food: sf }));
                    const _existingDayFoods = (context.selectedFoods || []).map((sf: any) => ({ slot: '', food: sf }));
                    const _allMeals = [..._existingDayFoods, ..._existingSlotFoods];
    ...
    private hasAffinityConflict(food: any, dayMeals: any[], slotName: string): boolean {
        const slotSelectedFoods = dayMeals.filter(m => m.slot === slotName).map(m => m.food).filter(Boolean)
        const daySelectedFoods = dayMeals.map(m => m.food).filter(Boolean)
        const context = { selectedFoods: daySelectedFoods, slotSelectedFoods: slotSelectedFoods }
        for (const rule of this.rules) {
            if (rule.rule_type === 'affinity' && rule.is_active) {
                if (this.checkAffinityRule(rule, food, context) === -Infinity) return true
            }
        }
        return false
    }
    ...
    private checkAffinityRule(rule: PlanningRule, food: any, context: any): number {
        const rawDef = rule.definition as any; const def = rawDef?.data || rawDef || {}
        ...
        if (def.association === 'forbidden') {
            ...
            // FORBIDDEN ALWAY MEANS HARD BAN (-INFINITY)
            // Even if the UI saved probability as 0, a forbidden rule should forbid.
            return Number.NEGATIVE_INFINITY;
        }
    ...
    private matchesTarget(food: any, target: any): boolean {
        ...
        if (target.type === 'food_id') return food.id === target.value || food.name?.toLowerCase() === target.value?.toLowerCase()
        if (target.type === 'tag') { const tVal = (target.value || '').toLowerCase(); return food.tags?.some((t: string) => t.toLowerCase() === tVal); }
                            ...
                            if (targetMeal?.food) {
                                ...
                                const swapCandidates = this.eligibleFoods
                                    .filter(f => {
                                        ...
                                        if (this.hasTagConflict(f, slotTags)) return false
                                        if (this.hasAffinityConflict(f, dayMeals, slot)) return false
                            ...
                            if (addDev.isUnder) {
                                ...
                                const addCandidates = this.eligibleFoods
                                    .filter(f => {
                                        ...
                                        if (this.hasAffinityConflict(f, dayMeals, bestSlot)) return false
                                        if (!this.checkSeasonalityHard(f, this.today)) return false
                                        const weekCount = this.currentWeekFoods.filter(wf => wf.id === f.id).length
                                        if (this.hasReachedWeeklyCap(f, weekCount)) return false
                                        if (getEffectivePriority(f) === 0) return false
